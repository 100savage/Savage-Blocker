...
AUTO_UPDATE_PATH = Path("/etc/cron.monthly/savage-blocker")
AUTO_UPDATE_CATS_FILE = BLOCKER_DIR / "auto-update-categories"
...
LONG_CAT_MAP = {v: k for k, v in SHORT_CAT_MAP.items()}
...
class SavageBlockerApp:
    ...
    def _auto_update_enabled(self):
        return AUTO_UPDATE_PATH.exists()

    def _toggle_auto_update(self):
        if self._auto_update_enabled():
            try:
                AUTO_UPDATE_PATH.unlink()
                self._log("Monthly auto-update disabled")
            except Exception as exc:
                self._on_error(f"Failed to disable auto-update: {exc}")
                return
        else:
            if not is_linux():
                self._show_message("Unsupported", "Auto-update requires Linux with cron.", "warning")
                return
            if is_linux() and not check_root():
                self._show_message("Not Root", "Installing auto-update requires root privileges.", "warning")
                return
            dialog = CategoryDialog(
                self.root, self.BG, self.FG, self.ACCENT, self.BTN_BG, self.BTN_HOVER,
                self.CARD_BG, "#f9e2af" if self._theme == "dark" else "#df8e1d",
                title="Select categories for auto-update:",
                confirm_text="Save & Enable",
            )
            self.root.wait_window(dialog.dialog)
            if not dialog.result:
                return
            short_names = [SHORT_CAT_MAP[c] for c in dialog.result]
            try:
                AUTO_UPDATE_CATS_FILE.parent.mkdir(parents=True, exist_ok=True)
                AUTO_UPDATE_CATS_FILE.write_text("\n".join(short_names) + "\n")
                script = (
                    '#!/bin/sh\n'
                    f'CATS_FILE="{AUTO_UPDATE_CATS_FILE}"\n'
                    'if [ -f "$CATS_FILE" ]; then\n'
                    f'    exec "{sys.executable}" "{os.path.abspath(__file__)}" --update $(cat "$CATS_FILE")\n'
                    'else\n'
                    f'    exec "{sys.executable}" "{os.path.abspath(__file__)}" --update\n'
                    'fi\n'
                )
                AUTO_UPDATE_PATH.parent.mkdir(parents=True, exist_ok=True)
                AUTO_UPDATE_PATH.write_text(script)
                AUTO_UPDATE_PATH.chmod(0o755)
                self._log("Monthly auto-update enabled")
            except Exception as exc:
                self._on_error(f"Failed to enable auto-update: {exc}")
                return
        self.main.destroy()
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        theme_btn = Button(
            ...
            relief="flat", bd=0, padx=15, pady=6, height=2,
            cursor="hand2",
        )
        theme_btn.pack(fill="x")

        auto_fg = self.ACCENT if self._auto_update_enabled() else self.FG
        auto_text = "Disable Monthly\nAuto-Update" if self._auto_update_enabled() else "Enable Monthly\nAuto-Update"
        auto_card = Frame(bottom_frame, bg=self.CARD_BG, padx=12, pady=4)
        auto_card.grid(row=0, column=1, sticky="ew", padx=5)
        auto_btn = Button(
            auto_card, text=auto_text, command=self._toggle_auto_update,
            font=("Segoe UI", 10),
            bg=self.CARD_BG, fg=auto_fg,
            activebackground=self.BTN_HOVER, activeforeground=auto_fg,
            relief="flat", bd=0, padx=15, pady=6, height=2,
            cursor="hand2",
        )
        ...
        exit_card.grid(row=0, column=2, sticky="ew", padx=(5, 0))
        exit_btn = Button(
        ...
        if self._auto_update_enabled():
            self._log("Monthly auto-update is ENABLED")
    ...
    def _on_update(self):
        dialog = CategoryDialog(self.root, self.BG, self.FG, self.ACCENT, self.BTN_BG, self.BTN_HOVER, self.CARD_BG, "#f9e2af" if self._theme == "dark" else "#df8e1d")
    ...
    def _on_add(self):
        dialog = InputDialog(
            ...
            self.BG, self.FG, self.ACCENT, self.BTN_BG, self.BTN_HOVER, self.CARD_BG,
            "#f9e2af" if self._theme == "dark" else "#df8e1d",
        )
        self.root.wait_window(dialog.dialog)
        if dialog.result:
            try:
    ...
    def _show_message(self, title, message, mode="info", parent=None):
        warning_fg = "#f9e2af" if self._theme == "dark" else "#df8e1d"
        d = MessageDialog(parent or self.root, title, message, mode, self.BG, self.FG, self.ACCENT, self.BTN_BG, self.BTN_HOVER, self.CARD_BG, self.RED, warning_fg)
...
# ---------------------------------------------------------------------------
class MessageDialog:
    def __init__(self, parent, title, message, mode="info", bg="#1e1e2e", fg="#cdd6f4", accent="#89b4fa", btn_bg="#313244", btn_hover="#45475a", card_bg="#2a2a3c", red="#f38ba8", warning_fg="#f9e2af"):
        _reload_tkinter()
...
class CategoryDialog:
    def __init__(self, parent, bg, fg, accent, btn_bg, btn_hover, card_bg="#2a2a3c", warning_fg="#f9e2af", title="Select categories to block:", confirm_text="Start Blocking"):
        _reload_tkinter()
        self.card_bg = card_bg
        self.warning_fg = warning_fg
        self.dialog = Toplevel(parent)
    ...
    def _on_confirm(self):
        ...
        if not self.result:
            d = MessageDialog(self.dialog, "No Selection", "Please select at least one category.", "warning", self.bg, self.fg, self.accent, self.btn_bg, self.btn_hover, self.card_bg, "#f38ba8", self.warning_fg)
...
class InputDialog:
    def __init__(self, parent, title, prompt, bg, fg, accent, btn_bg, btn_hover, card_bg="#2a2a3c", warning_fg="#f9e2af"):
        _reload_tkinter()
        self.card_bg = card_bg
        self.warning_fg = warning_fg
        self.dialog = Toplevel(parent)
    ...
    def _on_ok(self):
        ...
        if not val:
            d = MessageDialog(self.dialog, "Empty", "Please enter a domain.", "warning", self.bg, self.fg, self.accent, self.btn_bg, self.btn_hover, self.card_bg, "#f38ba8", self.warning_fg)
            self.dialog.wait_window(d.dialog)
            return
        self.result = val
        self.dialog.destroy()
...
def cli_main():
    ...
    try:
        if args.setup:
            ...
            categories = [LONG_CAT_MAP[c] for c in args.update] if args.update else list(
